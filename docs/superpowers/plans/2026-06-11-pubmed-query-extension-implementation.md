...
describe("parseGeneratedQuery", () => {
  it("parses a valid generated query JSON string", () => {
    const result = parseGeneratedQuery(
      JSON.stringify({
  ...
  it("throws a readable error when required fields are missing", () => {
    expect(() => parseGeneratedQuery('{"query":"asthma"}')).toThrow(
      "LLM 응답 형식이 올바르지 않습니다."
    );
  });

  it("throws a readable error when JSON is invalid", () => {
    expect(() => parseGeneratedQuery("not json")).toThrow(
      "LLM 응답을 JSON으로 해석할 수 없습니다."
    );
  });
});
````
...
describe("classifyProviderError", () => {
  it("classifies network failures", () => {
    expect(
      classifyProviderError(new TypeError("Failed to fetch"), "ollama")
    ).toEqual({
  ...
  it("classifies missing model errors", () => {
    expect(
      classifyProviderError(new Error("model not found"), "lmstudio").title
    ).toBe("모델을 사용할 수 없습니다.");
  });
});
```
...
function isConfidence(
  value: unknown
): value is MeshTermCandidate["confidence"] {
...
export function classifyProviderError(
  ...
  if (
    ...
    return {
      ...
      message:
        "설정에서 모델명을 확인하고, 로컬 런타임에 해당 모델이 설치되어 있는지 확인하세요.",
      debug,
    };
  }
...
describe("ollamaProvider", () => {
  ...
  it("tests connection with configured endpoint and model", async () => {
    vi.stubGlobal(
      "fetch",
      vi.fn(async () => new Response(JSON.stringify({ model: "gemma3" })))
    );
...
export const lmStudioProvider: LlmProvider = {
  ...
  async generatePubMedQuery(
    ...
    const response = await fetch(
      `${trimEndpoint(config.endpoint)}/chat/completions`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
    ...
    if (!content) {
      throw new Error(
        "LM Studio response JSON is missing choices[0].message.content"
      );
    }
...
export async function getSettings(): Promise<StoredSettings> {
  if (hasChromeStorage()) {
    ...
    return {
      ...defaultSettings,
      ...(result[STORAGE_KEY] as Partial<StoredSettings> | undefined),
    };
  }

  const raw = localStorage.getItem(STORAGE_KEY);
  return raw
    ? { ...defaultSettings, ...(JSON.parse(raw) as Partial<StoredSettings>) }
    : defaultSettings;
}
...
export function Onboarding({ initialSettings, onComplete }: OnboardingProps) {
  ...
  return (
    <section className="panel">
      ...
      <p className="muted">
        이 익스텐션은 로컬 LLM을 포함하지 않습니다. Ollama 또는 LM Studio를
        설치하고 모델을 실행한 뒤 연결하세요.
      </p>

      <div className="segmented" role="tablist" aria-label="Provider 선택">
        <button
          className={providerId === "ollama" ? "active" : ""}
          onClick={() => selectProvider("ollama")}
        >
          Ollama
        </button>
        <button
          className={providerId === "lmstudio" ? "active" : ""}
      ...
      <div className="guide">
        ...
        <p>
          4B급은 저사양 기기, 7B-8B급은 일반 노트북, 12B-14B급은 성능 여유가
          있는 환경에 권장합니다. Gemma, Qwen, Llama, Mistral 계열 instruct
          모델을 추천합니다.
        </p>
      </div>
